...
function mTranskeyObj(inputObj, width, div, keyType, keyboardType, dataType){
	...
	if(!useSession) {
		this.keyIndex = document.getElementById("keyIndex_"+inputObj.id).value;
		document.getElementById("keyboardType_"+inputObj.id).value = this.keyboardType;
		document.getElementById("fieldType_"+inputObj.id).value = this.fieldType;
	}
	...
	this.setUrl = function(){
		...
		if(transkey_divType==0){			
			if(this.keyboardType=="numberMobile"){
				...
				this.numberDiv.style.backgroundImage="url('"+numberImg.src+"')";
				...
				quertyImg.onload = function(){
					...
					self.upperDiv.style.backgroundImage="url('"+url+"')";
					...
					self.specialDiv.style.backgroundImage="url('"+url+"')";
				...
				this.lowerDiv.style.backgroundImage="url('"+quertyImg.src+"')";
	...
	this.checkInitTime = function(){
		...
		if(elapsedTime > limitTime) {
			...
			document.getElementById("initTime").value = initTime;
			return;
		}
	}
	...
	this.setBgImageForDivType1 = function(type){
		...
		if(type=="single"){
			for(var i=1; 5>i; i++){
				for(var j=0; 4>j; j++){
					try{
						this.div.childNodes[i].childNodes[j].childNodes[0].style.backgroundImage="url('"+img.src+"')";
					}catch(e){
...
//			}
			...
			for(var j=0; 3>j; j++){
				try{
					this.div.childNodes[5].childNodes[j].childNodes[0].style.backgroundImage="url('"+img.src+"')";
				}catch(e){
...
mTranskeyObj.prototype.getCipherData = function(xecureRandomData, crtType){
	...
	if(!useSession) {
		var seedKey = document.getElementById("seedKey").value;
...
function mTranskey(){
		...
		for(var i=0;transkey.objs.length>i;i++){
			...
			if(HM.value.length==0){
				...
				if(!useSession){
					...
					document.getElementById("seedKey").value = encSessionKey;
				}
	 ...
	 this.setHiddenField = function(inputObj, ExE2E){
		...
		if(!useSession) {
			...
			if(obj.children.seedKey==null){
				...
				seedKey.setAttribute("id", "seedKey");
				seedKey.setAttribute("name", "seedKey");
			...
			if(obj.children.initTime==null){
				...
				hidInitTime.setAttribute("id", "initTime");
				hidInitTime.setAttribute("name", "initTime");
			...
			sessionKey.setAttribute("id", "keyIndex_"+inputObj.id);
			sessionKey.setAttribute("name", "keyIndex_"+inputObj.id);
			...
			hidkeyboardType.setAttribute("id", "keyboardType_"+inputObj.id);
			hidkeyboardType.setAttribute("name", "keyboardType_"+inputObj.id);
			...
			hidfieldType.setAttribute("id", "fieldType_"+inputObj.id);
			hidfieldType.setAttribute("name", "fieldType_"+inputObj.id);
	...
	'</div>'+
	'<div class="dv_'+cssText+'_div_2 dv_'+cssText+'_div2_Height">'+
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_div_a dv_'+cssText+'_div2_Height" id="mtk_cp" '+startEvent+'="mtk.cap(event, this);" ><div class="dv_'+cssText+'_div2_2  dv_'+cssText+'_div2_2_qTop" style="background-position:-2px -154px;">'+setTalkbackImg(id, '쉬프트', false)+'</div></div>'+
		...
		'<div class="dv_'+cssText+'_div2 dv_'+cssText+'_div_a dv_'+cssText+'_end dv_'+cssText+'_div2_Height" id="mtk_del" '+startEvent+'="mtk.del(event, this);"><div class="dv_'+cssText+'_div2_2  dv_'+cssText+'_div2_2_qTop" style="background-position:-333px -154px;width:40px;">'+setTalkbackImg(id, '삭제', false)+'</div></div>'+
	'</div>'+
	'<div class="dv_'+cssText+'_div_2 dv_'+cssText+'_end dv_'+cssText+'_div2_Height">'+
		'<div class="dv_'+cssText+'_div3 dv_'+cssText+'_div_a dv_'+cssText+'_div2_Height"  id="mtk_sp" '+startEvent+'="mtk.sp(event, this);"><div class="dv_'+cssText+'_div3_2 dv_'+cssText+'_div3_3_qTop" style="width:42px;height:27px;background-position:-31px -211px;">'+setTalkbackImg(id, '특수키', false)+'</div></div>'+
		'<div class="dv_'+cssText+'_div3 dv_'+cssText+'_div2_Height"  '+startEvent+'="mtk.clear(event, this);"><div class="dv_'+cssText+'_div3_3 dv_'+cssText+'_div3_3_qTop" style="background-position:-130px -212px;">'+setTalkbackImg(id, '전체삭제', false)+'</div></div>'+
		'<div class="dv_'+cssText+'_div3 dv_'+cssText+'_div_b dv_'+cssText+'_end dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.done(event, this);"><div class="dv_'+cssText+'_div3_2 dv_'+cssText+'_div3_3_qTop" style="width:75px;height:27px;background-position:-263px -211px;">'+setTalkbackImg(id, '입력완료', false)+'</div></div>'+
	'</div>';
	...
	'</div>'+
	'<div class="dv_'+cssText+'_div_2 dv_'+cssText+'_end dv_'+cssText+'_div2_Height">'+
		'<div class="dv_'+cssText+'_div3 dv_'+cssText+'_div_a dv_'+cssText+'_div2_Height"  '+startEvent+'="mtk.del(event, this);"><div class="dv_'+cssText+'_div2_2 dv_'+cssText+'_div2_2_nTop" style="background-position:-33px -203px;">'+setTalkbackImg(id, '삭제', false)+'</div></div>'+
		'<div class="dv_'+cssText+'_div3 dv_'+cssText+'_div2_Height"  '+startEvent+'="mtk.clear(event, this);"><div class="dv_'+cssText+'_div3_3 dv_'+cssText+'_div3_3_nTop" style="background-position:-114px -206px;">'+setTalkbackImg(id, '전체삭제', false)+'</div></div>'+
		'<div class="dv_'+cssText+'_div2  dv_'+cssText+'_div_b dv_'+cssText+'_end dv_'+cssText+'_div2_Height" '+startEvent+'="mtk.done(event, this);"><div class="dv_'+cssText+'_div3_2 dv_'+cssText+'_div3_3_nTop" style="background-position:-269px -206px;">'+setTalkbackImg(id, '확인', false)+'</div></div>'+
	'</div>';
		if(showLicense&&licenseType!="")
			layout += '<div class="dv_'+cssText+'_licenseType" style="position : absolute;margin: 5px; color : red;">' + licenseType + " (" + licExpiredDate + ")" + '</div>';
	...
	function setTalkbackImg(id, alt, style){
		...
		if(mtk_useTalkBack){
			if(style){
				img = '<p href="#none" style="display:block; width:100%;height:100%;margin-top:0px;margin-bottom:0px;position:absolute;top:0px;left:0px;"><img tabindex="0" style="width:100%;height:100%;position:absolute;top:0px;left:0px;" src="'+transkey_url+'/images/0.png" alt="'+alt+'"></p>';
			}else{
				img = '<p href="#none" style="display:block; width:100%;height:100%;margin-top:0px;margin-bottom:0px;"><img tabindex="0" style="width:100%;height:100%" src="'+transkey_url+'/images/0.png" alt="'+alt+'"></p>';
			}
...
mTranskey.prototype.onKeyboard = function(inputObj){
	 ...
	 if(onKeyboard_allocate||!this.now.allocate) {
		 if(!useSession){
			 var hidKeyIndex = document.getElementById("keyIndex_"+inputObj.id);
